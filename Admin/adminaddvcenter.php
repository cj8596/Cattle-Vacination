<?php
    ...
    if(isset($_POST['submit'])){
        ...
        {
            $rs=mysqli_query($connection, "SELECT vc_id FROM v_center WHERE vc_id='$vc_id'");
            if(mysqli_num_rows($rs)>0)
            {
                echo '<script type="text/javascript"> alert("Vaccine Center already exists.")</script>';
            }
            else
            {
                $insert_query=mysqli_query($connection, "INSERT INTO v_center(vc_id, vc_name, vc_mobile, vc_address, vc_pincode, vc_password) VALUES('$vc_id', '$vc_name', '$vc_mobile', '$vc_address', '$vc_pincode', '$vc_password')");
                if($insert_query){
                    echo '<script type="text/javascript"> alert("Vaccine Center Added Successfully.");
                    window.location.replace("adminaddvcenter.php")</script>';
                }
                else{
                    echo "Error".mysqli_error($connection);
                    echo '<script>window.location.replace("adminaddvcenter.php")</script>';
                }
            }
        }else{
		...
		<div class="wrap-contact100">
			<form class="contact100-form validate-form" method="post" action="" id="validate-form">
				<span class="contact100-form-title">
					Add Vaccination Center Information
				</span>

				<div class="wrap-input100 validate-input bg1">
					<span class="label-input100">Center ID</span>
					<input class="input100" id="vc_id" type="text" name="vc_id" placeholder="Enter Center ID" required>
				</div>
				<div class="wrap-input100 validate-input bg1">
					<span class="label-input100">Center Name</span>
					<input class="input100" id="vc_name" type="text" name="vc_name" placeholder="Enter Center Name"  required>
				</div>
				<div class="wrap-input100 validate-input bg1">
					<span class="label-input100">Center Address</span>
					<input class="input100" id="vc_address" type="text" name="vc_address" placeholder="Enter Center Address" required>
				</div>
				
				<div class="wrap-input100 validate-input bg1 rs1-wrap-input100">
					<span class="label-input100">Center Pincode</span>
					<input class="input100" type="number" id="vc_pincode" name="vc_pincode" placeholder="Enter Pincode" required>
				</div>
				<div class="wrap-input100 validate-input bg1 rs1-wrap-input100">
					<span class="label-input100">Center Phone Number</span>
					<input class="input100" type="number" id="vc_mobile" name="vc_mobile" placeholder="Enter Phone Number" required>
				</div>
                <div class="wrap-input100 validate-input bg1">
